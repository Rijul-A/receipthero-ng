...
export class PaperlessClient {
  ...
  async findExistingTag(tagName: string): Promise<Tag | null> {
    ...
    try {
      const response = await this.fetchApi(
        `/tags/?name__iexact=${encodeURIComponent(normalizedName)}`,
      )
      const data = (await response.json()) as any
    ...
    try {
      ...
      if (error.message?.includes('unique constraint') || error.message?.includes('400')) {
        const retryRes = await this.fetchApi(
          `/correspondents/?name__icontains=${encodeURIComponent(name)}`,
        )
        const retryData = (await retryRes.json()) as any
        const retryExisting = retryData.results.find(
  ...
  async getUnprocessedDocuments(
    ...
    const processedTag = tags.find(
      (t: any) => t.name.toLowerCase() === processedTagName.toLowerCase(),
    )
    ...
    if (useDocumentType) {
      ...
      const receiptTag = tags.find(
        (t: any) => t.name.toLowerCase() === receiptTagName.toLowerCase(),
      )
      if (receiptTag) {
    ...
    while (nextUrl) {
      ...
      if (data.next) {
        ...
        const pathname = url.pathname.startsWith('/api') ? url.pathname.substring(4) : url.pathname
  ...
  async getDocumentTypes(): Promise<Array<{ id: number; name: string }>> {
    const now = Date.now()
    if (
      this.documentTypeCache.size > 0 &&
      now - this.lastDocumentTypeRefresh < this.DOCUMENT_TYPE_CACHE_LIFETIME
    ) {
  ...
  async getCustomFields(): Promise<Array<{ id: number; name: string; data_type: string }>> {
    const now = Date.now()
    if (
      this.customFieldCache.size > 0 &&
      now - this.lastCustomFieldRefresh < this.CUSTOM_FIELD_CACHE_LIFETIME
    ) {
  ...
  async findCustomFieldByName(
    name: string,
  ): Promise<{ id: number; name: string; data_type: string } | null> {
    const normalizedName = name.toLowerCase()

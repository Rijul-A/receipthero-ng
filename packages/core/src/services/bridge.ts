...
function receiptToMarkdown(receipt: any): string {
  ...
  const allTags = [...(receipt.suggested_tags || []), receipt.vendor?.split(' ')[0], year].filter(
    Boolean,
  )
...
export async function processPaperlessDocument(
  ...
  if (retryQueue && attemptNum > 1) {
    ...
    await reporter.report('workflow:retry', {
      documentId,
      attempts: attemptNum,
      progress: 0,
      status: 'retrying',
  ...
  try {
    ...
    if (!receipt) {
      ...
      let receipts: any[]
      try {
        receipts = await extractReceiptData(fileBuffer, adapter, {
          existingTags: existingTagNamesLegacy,
        })
      ...
      if (receipts.length === 0) {
        ...
        await reporter.report('receipt:skipped', {
          documentId,
          progress: 100,
          message: 'No receipt data found (skipped)',
        })
    ...
    if (config.processing.currencyConversion?.enabled && receipt.currency && receipt.date) {
      ...
      try {
        ...
        if (result) {
          ...
          docLogger.warn(
            `⚠ Currency conversion failed - could not fetch exchange rates for ${receipt.currency}`,
          )
        }
      } catch (error: any) {
    ...
    if (config.processing.autoTag && receipt.suggested_tags?.length) {
      ...
      const { tagIds: suggestedTagIds, errors: tagErrors } = await client.processTags(
        receipt.suggested_tags,
      )
    ...
    const newTitleLegacy =
      receipt.title || `${receipt.vendor} - ${receipt.amount} ${receipt.currency}`
    ...
    if (retryQueue) {
      if (await retryQueue.shouldGiveUp(documentId)) {
        ...
        await reporter.report('receipt:failed', {
          documentId,
        ...
        await reporter.report('receipt:retry', {
          documentId,
          message: errorMessage,
          attempts: attemptNum,
          progress: 0,
...
export async function runAutomation(): Promise<{
  ...
  for (const workflow of workflowsList) {
    ...
    try {
      ...
      logger.error(`Failed to fetch documents for workflow ${workflow.name}`, {
        error: error.message,
      })
    }
  }
  ...
  for (const item of readyForRetry) {
    ...
    const allTags = await client.getTags()
    const tagNames =
      doc.tags?.map((id: number) => allTags.find((t: any) => t.id === id)?.name).filter(Boolean) ||
      []
 ...
 */
async function runLegacyAutomation(
  client: PaperlessClient,
  adapter: AIAdapter,
  retryQueue: RetryQueue,
  config: any,
  ...
  for (const legacyDoc of unprocessed) {
    await reporter.report('receipt:detected', {

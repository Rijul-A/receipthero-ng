...
export function dataToMarkdown(data: Record<string, unknown>, workflowName: string): string {
  const entries = Object.entries(data)
    .filter(
      ([key]) => !['line_items', 'suggested_tags', 'conversions', 'title', 'summary'].includes(key),
    )
  ...
  try {
    ...
    if (!extractedData) {
      ...
      if (items.length === 0) {
        ...
        await reporter.report('workflow:skipped', {
          documentId,
          progress: 100,
    ...
    if (mapping.correspondentField && extractedData[mapping.correspondentField]) {
      const correspondentId = await client.getOrCreateCorrespondent(

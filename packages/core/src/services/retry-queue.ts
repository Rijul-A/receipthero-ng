...
export class RetryQueue {
  ...
  async add(documentId: number, error: string): Promise<void> {
    ...
    logger.warn(
      `Document ${documentId} failed (attempt ${attempts}/${this.maxRetries}), will retry in ${this.formatDelay(delay)}`,
    )
  }
  ...
  async remove(documentId: number): Promise<void> {
    await db.delete(schema.retryQueue).where(eq(schema.retryQueue.documentId, documentId)).run()
  }
  ...
  async size(): Promise<number> {
    const result = await db.select({ count: schema.retryQueue.id }).from(schema.retryQueue).all()
  ...
  async getAll(): Promise<schema.RetryQueueEntry[]> {
    return await db.select().from(schema.retryQueue).all()
  }

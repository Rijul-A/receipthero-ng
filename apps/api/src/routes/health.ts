  ...
  if (config && isPaperlessConfigured(config)) {
    try {
      ...
      const processedTag = tags.find(
        (t: any) => t.name.toLowerCase() === config.processing.processedTag.toLowerCase(),
      )
      const receiptTag = tags.find(
        (t: any) => t.name.toLowerCase() === config.processing.receiptTag.toLowerCase(),
      )
      const failedTag = tags.find(
        (t: any) => t.name.toLowerCase() === config.processing.failedTag.toLowerCase(),
      )
      ...
      if (receiptTag) {
        const res = await fetch(
          `${config.paperless.host.replace(/\/$/, '')}/api/documents/?tags__id__all=${receiptTag.id}&page_size=1`,
          {
      ...
      if (processedTag) {
        const res = await fetch(
          `${config.paperless.host.replace(/\/$/, '')}/api/documents/?tags__id__all=${processedTag.id}&page_size=1`,
          {
            headers: { Authorization: `Token ${config.paperless.apiKey}` },
          },
        )
      ...
      if (failedTag) {
        const res = await fetch(
          `${config.paperless.host.replace(/\/$/, '')}/api/documents/?tags__id__all=${failedTag.id}&page_size=1`,
          {
            headers: { Authorization: `Token ${config.paperless.apiKey}` },
          },
        )
      ...
      status.status = 'unhealthy'
      errors.push(
        `Paperless connection/stats failed: ${error instanceof Error ? error.message : String(error)}`,
      )
    }
  } else {
    status.checks.paperlessConnection = 'error'

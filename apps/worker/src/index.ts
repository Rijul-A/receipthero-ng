  ...
  while (!isShuttingDown) {
    try {
      ...
      if (isPaused) {
        ...
        logger.info(
          `Worker paused${state.pauseReason ? `: ${state.pauseReason}` : ''}. Waiting for resume...`,
        )
      ...
      try {
        if (wasTriggered) {
          ...
          logger.info(
            `Running scheduled automation cycle (${Math.round(timeSinceLastScan / 1000)}s since last scan)...`,
          )
        }
        ...
        logger.info(
          `Automation cycle complete. Next scan in ${scanInterval / 1000}s (unless manually triggered)`,
        )
      } finally {

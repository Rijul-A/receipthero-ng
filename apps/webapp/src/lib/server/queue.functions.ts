 ...
 */
export const retryAllQueue = createServerFn({ method: 'POST' }).handler(
  async (): Promise<QueueActionResponse> => {
 ...
 */
export const getSkippedDocuments = createServerFn({ method: 'GET' }).handler(
  async () => {
    return apiCall<{ success: boolean; count: number; items: Array<any> }>(
      '/api/queue/skipped',
    )
  },
)
 ...
 */
export const clearSkippedDocuments = createServerFn({ method: 'POST' }).handler(
  async (): Promise<QueueActionResponse> => {
